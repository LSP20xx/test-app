...
router.post('/auth/register', async (req, res) => {
  try {
    const { username, password, firstName, lastName, email, dateOfBirth, gender, accountNumber } = req.body;
...
router.post('/auth/biometrics', isAuthenticated, async (req, res) => {
  try {
    ...
    const detectedFaces = await faceClient.face.detectWithStream(faceScanBuffer, {
      returnFaceId: true,
    ...
    if (detectedFaces.length === 0) {
      ...
      return res.status(400).send('No faces detected in the provided face scan.');
    }
...
router.post('/auth/upload-id', isAuthenticated, upload.fields([{ name: 'governmentIDPhoto' }, { name: 'selfiePhoto' }]), async (req, res) => {
  try {
    ...
    console.log('Identification documents uploaded successfully:', identification);
