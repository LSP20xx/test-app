...
router.post('/upload-consent', isAuthenticated, upload.single('consentDocument'), async (req, res) => {
  try {
    ...
    console.log('Parental consent document uploaded successfully:', parentalConsent);
    res.send('Parental consent document uploaded successfully.');
  } catch (error) {
    console.error('Error uploading parental consent document:', error.message);
